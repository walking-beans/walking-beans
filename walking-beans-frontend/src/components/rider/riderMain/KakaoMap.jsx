...
const KAKAO_MAP_API_KEY_LEO = "1cfadb6831a47f77795a00c42017b581"; // 본인 카카오 API 키
...
const getDistance = (lat1, lng1, lat2, lng2) => {
    console.log("parseFloat(store.storeLatitude), parseFloat(store.storeLongitude) : " + parseFloat(lat2) + ", " + parseFloat(lng2));
                ...
                (position) => {
                    ...
                    apiRiderService.getStoreInfoInRiderMain((newOrders) => {
                            ...
                            const filtered = untakenOrders.filter((store) =>
                                getDistance(lat, lng, parseFloat(store.storeLatitude), parseFloat(store.storeLongitude)) <= 10
                            );
            ...
            window.kakao.maps.load(() => {
                ...
                filteredStores.forEach((store) => {
                    ...
                    const marker = new window.kakao.maps.Marker({
                        position: new window.kakao.maps.LatLng(parseFloat(store.storeLatitude), parseFloat(store.storeLongitude)),
                        map: map,
