...
public class WebSocketChatHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        Message messageDTO = objectMapper.readValue(message.getPayload(), Message.class);

        // 닉네임을 처음 보낼 경우 세션과 연결
        if (!userSessions.containsKey(messageDTO.getSender())) {
        ...
        // 수신자에게 메시지 전송
        if (messageDTO.getReceiver() != null && userSessions.containsKey(messageDTO.getReceiver())) {
